...
private constructor(
     ...
     *   server responded with an unexpected value).
     */
    fun tables(): Optional<List<Table>> = tables.getOptional("tables")
    ...
    fun validate(): GenericDocument = apply {
        ...
        state()
        status()
        tables().ifPresent { it.forEach { it.validate() } }
    ...
    internal fun validity(): Int =
        (if (id.asKnown().isPresent) 1 else 0) +
            ...
            (tables.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) +
    ...
    private constructor(
         ...
         */
        fun operation(): Optional<List<JsonValue>> = operation.getOptional("operation")
         ...
         * Used for best match union deserialization.
         */
        @JvmSynthetic internal fun validity(): Int = (operation.asKnown().getOrNull()?.size ?: 0)
    ...
    private constructor(
         ...
         *   the server responded with an unexpected value).
         */
        fun value(): Optional<List<Long>> = value.getOptional("value")
        ...
        internal fun validity(): Int =
            (if (confidence.asKnown().isPresent) 1 else 0) +
                (if (name.asKnown().isPresent) 1 else 0) +
                (value.asKnown().getOrNull()?.size ?: 0)
