...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { DataleonInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw DataleonInvalidDataException("`$name` is not set")
            is JsonNull -> throw DataleonInvalidDataException("`$name` is null")
            else -> throw DataleonInvalidDataException("`$name` is invalid, received $this")
        }
